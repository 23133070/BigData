...
df = spark.read.csv("hdfs://172.20.10.50:9000/input/car_data_clean.csv", header=True, inferSchema=True)
# rename columns
...
avg_by_year.write.mode("overwrite").parquet("hdfs://172.20.10.50:9000/output/avg_price_by_year")
...
max_min.write.mode("overwrite").parquet("hdfs://172.20.10.50:9000/output/max_min_price_by_year")

# 3. top5 cars
top5 = df.groupBy("car_name").count().orderBy(F.desc("count")).limit(5)
top5.show(20, truncate=False)
top5.write.mode("overwrite").parquet("hdfs://172.20.10.50:9000/output/top5_cars")
...
avg_origin.write.mode("overwrite").parquet("hdfs://172.20.10.50:9000/output/avg_price_by_origin")
